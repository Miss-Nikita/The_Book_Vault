...
const DetailLayout = () => {
  ...
  const { book } = useSelector((store) => store.BooksSlice);
  console.log(book);
  ...
  return (
    book && (
      <div className="px-20 pt-20 pb-10 flex">
        ...
        <button className="px-4 ml-6 py-2 bg-blue-500 rounded mt-3 text-white font-semibold">More Information</button>
        </a>
        {book.accessInfo.pdf && book.accessInfo.pdf.isAvailable && (
        <a href={book.accessInfo.pdf.acsTokenLink} download>
          <button className="mt-4 ml-10 px-4 py-2 bg-blue-500 text-white rounded">
            Download PDF
          </button>
        </a>
      )}
